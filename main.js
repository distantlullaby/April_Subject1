...
class LinkFixerPlugin extends obsidian_1.Plugin {
    async onload() {
    ...
    handlePaste(evt, editor, view) {
        ...
        new obsidian_1.Notice('链接路径已自动修复');
    }
    fixLinksInEditor(editor, view) {
        const currentFile = view.file;
        if (!currentFile) {
            new obsidian_1.Notice('请先打开一个文件');
            return;
        }
        const selection = editor.getSelection();
        if (!selection) {
            new obsidian_1.Notice('请先选择内容');
            return;
        }
        if (!selection.includes('[[') && !selection.includes('](')) {
            new obsidian_1.Notice('选区内未发现链接');
            return;
        }
        const fixedContent = this.fixLinksInContent(selection, currentFile);
        editor.replaceSelection(fixedContent);
        new obsidian_1.Notice('链接路径已修复');
    }
    fixLinksInContent(content, targetFile) {
    ...
    resolveLink(link, targetFile) {
        const vault = this.app.vault;
        const allFiles = vault.getFiles();
        let targetFileObj = allFiles.find(f => f.basename === link.target.replace(/\.md$/, '') ||
            f.name === link.target ||
            f.path === link.target);
        if (!targetFileObj) {
...
exports.default = LinkFixerPlugin;
